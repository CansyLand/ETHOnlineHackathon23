    ...
    BaseERC4337Account
{
    ...
    address public immutable accountAbstrEntryPoint;
    ...
    constructor(address _guardian, address entryPoint_) {
        ...
        accountAbstrEntryPoint = entryPoint_;
        guardian = _guardian;
    }
    ...
    function state() external view override returns (uint256) {
        return IEntryPoint(accountAbstrEntryPoint).getNonce(address(this), 0);
    }
    ...
    function _incrementNonce() internal {
        if (msg.sender != accountAbstrEntryPoint)
            IEntryPoint(accountAbstrEntryPoint).incrementNonce(0);
    }

    /// @dev Return the ERC-4337 entry point address
    function entryPoint() public view override returns (IEntryPoint) {
        return IEntryPoint(accountAbstrEntryPoint);
    }
    ...
    function isAuthorized(address caller) public view returns (bool) {
        ...
        if (caller == accountAbstrEntryPoint) return true;
